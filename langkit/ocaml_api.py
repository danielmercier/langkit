...
class OCamlAPISettings(AbstractAPISettings):
    ...
    def get_parse_fields(self, tpe):
        ...
        :rtype: list[ct.Field]
        """
        return tpe.get_parse_fields(lambda x: not x.abstract and not x.null)

    def get_properties(self, tpe):
        """
        Return the list of all properties that should be exposed to the user.
        The returned boolean for each field is True iff the name of the field
        (field.name.lower) is unique.

        :rtype: list[(ct.ASTNodeType, ct.Field, bool)]
        """
