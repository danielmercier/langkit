...
module ${ocaml_api.node_name(astnode)} : sig
  ...
  (**
   * Fold all fields of the given node. This skips any child that is None because
   * of a syntax error or because the field is optional
   *)

  val iter_fields :
    (${root_entity_type} -> unit) -> [< ${root_entity_type} ] -> unit
  (**
   * Iter all fields of the given node. This skips any child that is None because
   * of a syntax error or because the field is optional
   *)
